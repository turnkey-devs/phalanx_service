...
elif [[ "${1}" == "--cron" && "${2}" == "${COMMAND_ARR[3]}" ]]; then
	{
		echo '[WARN] Make sure you build before run this code'
		createScriptCRON "${COMMAND_ARR[2]}" "0 */4 * * *" # run every 4 hours
	}

elif [[ "${1}" == "--start" && "${2}" == "${COMMAND_ARR[3]}" ]]; then
	{
		npm run cli-js daily_take_account_snapshot >>"$LOG_DIR/${2}_$DATE.log" 2>"$LOG_DIR/${2}_$DATE.error.log"
	}
